...
function NetworkRow() {
  ...
  const wifiSsid = network.wifi ? createBinding(network.wifi, "ssid") : null
  ...
  const iconName = createMemo(() => {
    const p = primary()
    if (p === AstalNetwork.Primary.WIRED) return wiredIcon?.() ?? "network-wired-symbolic"
    if (p === AstalNetwork.Primary.WIFI) return wifiIcon?.() ?? "network-wireless-symbolic"
